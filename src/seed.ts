...
async function seed() {
  ...
  if (!tenant) {
    const rows = await dbc.insert(tenants).values({
  ...
  if (!user) {
    ...
    const rows = await dbc.insert(users).values({
      email: SEED_EMAIL,
  ...
  if (existingEventCount === 0) {
    ...
    if (apiKey) {
      await dbc.insert(rateLimitEvents).values([
        { tenantId: tenant.id, apiKeyId: apiKey.id, ipAddress: "192.168.1.1", endpoint: "/api/v1/data", method: "GET", statusCode: 200, requestDurationMs: 42, isBlocked: false, remainingQuota: 4999 },
        { tenantId: tenant.id, apiKeyId: apiKey.id, ipAddress: "192.168.1.1", endpoint: "/api/v1/data", method: "POST", statusCode: 200, requestDurationMs: 87, isBlocked: false, remainingQuota: 4998 },
        { tenantId: tenant.id, apiKeyId: apiKey.id, ipAddress: "10.0.0.5", endpoint: "/api/v1/auth", method: "POST", statusCode: 429, requestDurationMs: 12, isBlocked: true, remainingQuota: 0 },
        { tenantId: tenant.id, apiKeyId: apiKey.id, ipAddress: "192.168.1.1", endpoint: "/api/v1/data", method: "GET", statusCode: 200, requestDurationMs: 31, isBlocked: false, remainingQuota: 4997 },
        { tenantId: tenant.id, apiKeyId: apiKey.id, ipAddress: "203.0.113.1", endpoint: "/api/v1/admin", method: "GET", statusCode: 403, requestDurationMs: 5, isBlocked: true, remainingQuota: 4996 },
      ]);
    }
  }
  ...
  console.log(`  -d '{"email":"${SEED_EMAIL}","password":"${SEED_PASSWORD}"}' | python3 -m json.tool\n`);
  ...
  console.log(`curl -s ${BASE}/api-keys/keys -H '${AUTH}' | python3 -m json.tool\n`);
  console.log(`curl -s -X POST ${BASE}/api-keys/keys \\`);
  console.log(`  -H 'Content-Type: application/json' \\`);
  console.log(`  -H '${AUTH}' \\`);
  console.log(`  -d '{"name":"New Test Key","scopes":["read"]}' | python3 -m json.tool\n`);

  if (apiKey) {
    console.log(`curl -s ${BASE}/api-keys/keys/${apiKey.id} -H '${AUTH}' | python3 -m json.tool\n`);
    console.log(`curl -s -X PATCH ${BASE}/api-keys/keys/${apiKey.id} \\`);
    console.log(`  -H 'Content-Type: application/json' \\`);
    console.log(`  -H '${AUTH}' \\`);
    console.log(`  -d '{"description":"Updated description"}' | python3 -m json.tool\n`);
    console.log(`curl -s -X DELETE ${BASE}/api-keys/keys/${apiKey.id} -H '${AUTH}' | python3 -m json.tool\n`);
  }

  console.log("# --- Tenants ---");
  console.log(`curl -s ${BASE}/tenants -H '${AUTH}' | python3 -m json.tool\n`);
  console.log(`curl -s ${BASE}/tenants/${tenant.id} -H '${AUTH}' | python3 -m json.tool\n`);

  console.log("# --- Analytics ---");
  console.log(`curl -s '${BASE}/analytics?tenantId=${tenant.id}' -H '${AUTH}' | python3 -m json.tool\n`);
  ...
  console.log(`  -H 'Content-Type: application/json' \\`);
  console.log(`  -H '${AUTH}' \\`);
  console.log(`  -d '{"url":"https://example.com/webhook","events":["quota_warning"],"secret":"sekret"}' | python3 -m json.tool\n`);
  console.log(`curl -s ${BASE}/webhooks -H '${AUTH}' | python3 -m json.tool\n`);

  if (webhookList.length > 0) {
    console.log(`curl -s ${BASE}/webhooks/${webhookList[0]?.id} -H '${AUTH}' | python3 -m json.tool\n`);
    console.log(`curl -s -X PATCH ${BASE}/webhooks/${webhookList[0]?.id} \\`);
    console.log(`  -H 'Content-Type: application/json' \\`);
    console.log(`  -H '${AUTH}' \\`);
    console.log(`  -d '{"isActive":false}' | python3 -m json.tool\n`);
    console.log(`curl -s -X DELETE ${BASE}/webhooks/${webhookList[0]?.id} -H '${AUTH}' | python3 -m json.tool\n`);
  }
  ...
  console.log(`  curl -s ${BASE}/health | python3 -m json.tool`);
  console.log(`  curl -s ${BASE}/api-keys/keys -H "Authorization: Bearer \$TOKEN" | python3 -m json.tool`);
}
